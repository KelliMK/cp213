...
public class RTSliderView extends JPanel {
	...
	private class BaseSliderListener implements ChangeListener {
		...
		public void stateChanged(final ChangeEvent e) {
			...
			if (!source.getValueIsAdjusting()) {
				RTSliderView.this.model.setBase(RTSliderView.this.baseSlider.getValue());
			}
			// your code here 
		}
	}
	...
	private class HeightSliderListener implements ChangeListener {
		...
		public void stateChanged(final ChangeEvent e) {
			final JSlider source = (JSlider) e.getSource();
			if (!source.getValueIsAdjusting()) {
				RTSliderView.this.model.setHeight(RTSliderView.this.heightSlider.getValue());
			}
			// your code here
	...
	private class ValuesListener implements PropertyChangeListener {
		...
		public void propertyChange(final PropertyChangeEvent evt) {
			...
			// your code here
			RTSliderView.this.hypo.setText(DECIMAL_FORMAT.format(model.getHypotenuse()));
			RTSliderView.this.heightSlider.setValue((int) model.getHeight());
			RTSliderView.this.baseSlider.setValue((int) model.getBase());
		}
	}
	 ...
	 */
	private final JSlider baseSlider = new JSlider(JSlider.HORIZONTAL, 0, (int) RTModel.MAX_SIDE, 100);
	/**
	 * The slider to adjust the Right Triangle height.
	 */
	private final JSlider heightSlider = new JSlider(JSlider.VERTICAL, 0, (int) RTModel.MAX_SIDE, 100);
	/**
