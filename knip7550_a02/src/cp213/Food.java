...
public class Food implements Comparable<Food> {
    ...
    public static String originsMenu() {
    	...
    	for (int i = 0; i < ORIGINS.length; i++) {
    		System.out.println(String.format("% 2d", i) + " " + ORIGINS[i]);
    	}
    	return null;
    }
    ...
    public int compareTo(final Food target) {
    	int result = 0;
    	int firstComp = this.name.compareToIgnoreCase(target.name);
    	if (firstComp == 0) {
    		result = this.origin - target.origin;
    	} else {
    		result = firstComp;
    	}
    	return result;
    ...
    public void write(final PrintStream ps) {
    	String result = name + "|" + origin + "|" + isVegetarian + "|" + calories;
